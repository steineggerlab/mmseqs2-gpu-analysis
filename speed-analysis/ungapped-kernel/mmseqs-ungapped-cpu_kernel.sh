...
for QUERYBATCH in ${BENCHDIR}/${QUERY_NAME}.split/*; do
  ${WORKSPACE}/bin/mmseqs-cpu createdb "${QUERYBATCH}" "${QUERYBATCH}".cqdb --shuffle 0
	start_time=$(date +%s%3N)
	${WORKSPACE}/bin/mmseqs-cpu ungappedprefilter "${QUERYBATCH}".cqdb /dev/shm/targetdb_mask_pad ${BENCHDIR}/${TAG}.pref --max-seqs 4000 --threads 128 --prefilter-mode 1 --db-load-mode 2
	end_time=$(date +%s%3N)
