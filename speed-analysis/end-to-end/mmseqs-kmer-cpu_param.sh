...
### MMSEQS-KMER-CPU
${WORKSPACE}/bin/mmseqs-cpu createdb $DB /dev/shm/targetdb
${WORKSPACE}/bin/mmseqs-cpu createindex /dev/shm/targetdb ${BENCHDIR}/tmp
for QUERYBATCH in ${BENCHDIR}/${QUERY_NAME}.split/*; do
	start_time=$(date +%s%3N)
	${WORKSPACE}/bin/mmseqs-cpu easy-search "${QUERYBATCH}" /dev/shm/targetdb ${BENCHDIR}/${TAG}.aln ${BENCHDIR}/tmp --max-seqs 4000 --threads 128 -s 8.5 -e 10000 --db-load-mode 2
	end_time=$(date +%s%3N)
