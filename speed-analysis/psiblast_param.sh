...
for QUERYBATCH in ${BENCHDIR}/${QUERY_NAME}.split/*; do
	...
	${WORKSPACE}/bin/psiblast -db /dev/shm/targetdb.blast -query "${QUERYBATCH}"  -num_threads 128 -evalue 10000.0 -out ${BENCHDIR}/${TAG}.blastout -outfmt 6 -num_alignments 4000 -num_iterations ${ITERATIONS}
	end_time=$(date +%s%3N)
